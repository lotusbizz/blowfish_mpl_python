"""
blowfish/core.py - Реализация алгоритма Blowfish на Python 3.8+
"""
...
class Blowfish:
    """
    Реализация алгоритма Blowfish с поддержкой базовых операций шифрования/дешифрования
    """
    ...
    def __init__(self, key: bytes):
        ...
        self.P = list(self.P_INIT)
        self.S = [list(sbox) for sbox in self.S_INIT]

        self._key_expansion(key)

    def _key_expansion(self, key: bytes):
        """Процесс генерации подключей"""
        key_len = len(key)
        j = 0

        # XOR P-блоков с ключом
        for i in range(18):
            data = 0
            for k in range(4):
    ...
    def _feistel_function(self, x: int) -> int:
        """Функция Фейстеля"""
        h = (self.S[0][x >> 24] + self.S[1][(x >> 16) & 0xFF]) % 0x100000000
        h ^= self.S[2][(x >> 8) & 0xFF]
        h = (h + self.S[3][x & 0xFF]) % 0x100000000
        return h

    def encrypt_block(self, data: bytes) -> bytes:
        """Шифрование 64-битного блока данных"""
        if len(data) != 8:
    ...
    def _encrypt_block(self, left: int, right: int) -> Tuple[int, int]:
        ...
        for i in range(16):
            left ^= self.P[i]
            right ^= self._feistel_function(left)
    ...
    def decrypt_block(self, data: bytes) -> bytes:
        """Дешифрование 64-битного блока данных"""
        if len(data) != 8:
            raise ValueError("Block must be exactly 8 bytes")
...
class BlowfishCBC:
    """Реализация режима CBC с PKCS7 дополнением"""
    ...
    def encrypt(self, plaintext: bytes) -> bytes:
        ...
        for block in blocks:
            ...
            encrypted = self.bf.encrypt_block(xored)
    ...
    def decrypt(self, ciphertext: bytes) -> bytes:
        ...
        plaintext = b''
        for block in blocks:
            decrypted = self.bf.decrypt_block(block)
